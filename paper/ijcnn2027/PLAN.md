...
- [ ] Reconcile the report, CSV files, generated figures, and final result
      tables.
- [ ] Replace stale project-status statements in the report and README.
- [ ] Generate one authoritative table for the Fashion-MNIST MLP and CNN
      campaigns.
- [ ] Verify symmetric communication accounting for every method: uplink,
      replay/checkpoint downlink, headers, addresses, and total unicast bits.
- [ ] Confirm that baseline tuning uses only development partitions.
- [ ] Decide whether the existing three held-out seeds need to be extended.
- [ ] Freeze seeds, configurations, dependencies, and reproduction commands.

**Gate:** every number intended for the paper is generated by a frozen script
and traceable to a result artifact.
