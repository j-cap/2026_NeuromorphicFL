...
def table_tex(cells: list[dict[str, str]]) -> str:
    ...
    lines = [
        ...
        "\\begin{table*}[t]",
        "\\centering",
        "\\caption{\\actionrev{Fashion-MNIST MLP alignment audit. Values are "
        "mean $\\pm$ sample standard deviation over ten paired seeds and 31 "
        ...
        "$\\overline B_{\\mathcal A}$ & $\\overline L_{\\mathcal A}$ \\\\",
        "\\midrule",
    ]
    for cell in cells:
        lines.append(
            ...
            f"{pm(cell, 'sampled_event_curvature_factor', 1.0, 1)} & "
    ...
    lines.extend(
        [
            ...
            "\\eqref{eq:empirical-theorem-diagnostics}. "
            "$\\overline B_{\\mathcal A}$ and $\\overline L_{\\mathcal A}$ "
            "are the normalized $q_r$-weighted heterogeneity and "
            "local-mismatch terms.}",
            "\\end{table*}",
            "",
        ]
    )
