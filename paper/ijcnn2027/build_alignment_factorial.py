...
def table_tex(cells: list[dict[str, str]]) -> str:
    ...
    lines = [
        ...
        "\\begin{table}[t]",
        "\\centering",
        "\\caption{Fashion-MNIST MLP alignment audit. Values are mean $\\pm$ "
        "sample standard deviation over ten paired seeds and 31 replayed rounds "
        "per seed.}",
        ...
        "\\toprule",
        "Setting & $\\widehat{\\kappa}_{\\mathcal A}$ & $A_r>0$ & "
        "$\\Delta F_r<0$ & $A_r>0,\\Delta F_r\\geq0$ \\\\",
        " & & [\\%] & [\\%] & [\\%] \\\\",
        "\\midrule",
    ]
    for cell in cells:
        lines.append(
            ...
            f"{pm(cell, 'positive_without_descent_fraction', 100.0, 1)} \\\\"
        )
    lines.extend(
        [
            "\\bottomrule",
            ...
            "\\toprule",
            "Setting & $\\overline B_{\\mathcal A}$ & "
            "$\\overline L_{\\mathcal A}$ & "
            "$10^3\\overline{\\mathcal R}_{\\mathcal A}$ \\\\",
            "\\midrule",
        ]
    )
    for cell in cells:
        lines.append(
            f"{labels[cell['regime']]}, $E={cell['local_steps']}$ & "
            f"{pm(cell, 'heterogeneity_ratio', 1.0, 2)} & "
            f"{pm(cell, 'local_drift_ratio', 1.0, 1)} & "
            f"{pm(cell, 'mean_curvature_remainder', 1000.0, 1)} \\\\"
        )
    lines.extend(
        [
            "\\bottomrule",
            "\\end{tabular*}",
            "\\vspace{2pt}",
            "\\parbox{\\columnwidth}{\\scriptsize $\\overline B_{\\mathcal A}$ "
            "and $\\overline L_{\\mathcal A}$ are the $q_r$-weighted sums of "
            "$B_r$ and $L_r$, divided by $\\sum_{r\\in\\mathcal A}q_r"
            "\\lVert\\vect{g}^r\\rVert_2^2$. "
            "$\\mathcal R_r=F(\\vect{w}^{r+1})-F(\\vect{w}^r)+q_rA_r$ is the "
            "observed curvature remainder.}",
            "\\end{table}",
            "",
        ]
    )
