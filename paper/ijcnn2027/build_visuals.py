...
def main_table(grouped: dict[str, list[dict[str, str]]]) -> str:
    ...
    lines = [
        ...
        "\\begin{table*}[t]",
        "\\centering",
        "\\caption{Ten-seed headline comparisons under conservative bidirectional "
        "unicast accounting. Values are mean $\\pm$ sample standard deviation. "
        "The CIFAR-10 block also includes the dense headline and EF-TopK class-wise "
        "references. Bold marks the best mean per benchmark and metric; lower "
        "traffic is better.}",
