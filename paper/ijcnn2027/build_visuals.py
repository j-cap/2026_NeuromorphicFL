...
def main_table(grouped: dict[str, list[dict[str, str]]]) -> str:
    ...
    entries = [
        next(row for row in resnet_rows if row["method"] == "event"),
        next(row for row in resnet_rows if row["method"] == "dense"),
        next(row for row in resnet_rows if row["method"] == "ef_topk"),
    ]
    best_accuracy = max(value(row, "final_test_accuracy_mean") for row in entries)
    best_worst = max(value(row, "final_worst_class_accuracy_mean") for row in entries)
    best_traffic = min(value(row, "unicast_total_Mbit_mean") for row in entries)
