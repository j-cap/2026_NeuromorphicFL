...
def main_table(grouped: dict[str, list[dict[str, str]]]) -> str:
    ...
    lines = [
        ...
        "\\begin{table*}[t]",
        "\\centering",
        "\\caption{\\spacerev{Primary frozen configurations. Values are mean "
        "$\\pm$ sample standard deviation over ten matched seeds. Bold marks the "
        "best mean per benchmark and metric. Lower traffic is better.}}",
    ...
    for index, (key, _title, _architecture, _ylim) in enumerate(CORE_PANELS):
        ...
        entries.append((r"\newrev{Quality-selected setting}", quality))
        if key == "cifar_cnn":
            entries.append(("Class-wise reference", cifar_ef))
        entries.append((r"\newrev{Traffic-matched setting}", traffic))
    ...
    entries = [
        ...
        (r"\newrev{Quality-selected setting}", next(row for row in resnet_rows if row["method"] == "ef_topk")),
        (r"\newrev{Quality-selected setting}", next(row for row in resnet_rows if row["method"] == "sign_ef")),
        (r"\newrev{Quality-selected setting}", next(row for row in resnet_rows if row["method"] == "strom")),
    ]
    best_accuracy = max(value(row, "final_test_accuracy_mean") for _, row in entries)
    best_worst = max(value(row, "final_worst_class_accuracy_mean") for _, row in entries)
    best_traffic = min(value(row, "unicast_total_Mbit_mean") for _, row in entries)
