...
% Historical revision wrappers remain only to avoid rewriting reviewed text.
% They produce ordinary black manuscript text.
\newcommand{\rev}[1]{#1}
\newcommand{\prov}[1]{#1}
\newcommand{\newrev}[1]{#1}
...
\begin{align}
 ...
 &\leq
 \frac{F(\vect{w}^1)-F_{\inf}}{\kappa Q_R}
 + \frac{\sum_{r=1}^Rq_r\beta_r}{\kappa Q_R} \notag\\
 &\quad +\frac{LM}{2\kappa Q_R}
...
\begin{figure*}[t]
 \centering
 \includegraphics[width=0.94\textwidth]{figures/resnet14_communication_trajectory.pdf}
 \caption{\spacerev{CIFAR-10 ResNet-14 learning curves over ten matched seeds.
 Lines and shaded bands show the mean and pointwise 95\% confidence interval.
 Panel (a) plots held-out cross-entropy against communication rounds, with the
 dotted line marking round 1,800. Panel (b) plots held-out accuracy against
 cumulative bidirectional traffic. Open circles mark round 1,800 and method
 symbols mark the common 3,000-round endpoint.}}
...
\spacerev{Figure~\ref{fig:resnet-communication-trajectory}(a) shows that
Event-FedAvg reaches its minimum mean held-out cross-entropy near round 1,800.
Its training loss continues to decrease thereafter, while held-out loss rises,
which indicates a generalization plateau rather than incomplete optimization.
Dense FedAvg approaches a plateau, whereas EF-TopK and Sign-EF continue to
improve at round 3,000. Panel (b) shows that Event-FedAvg reaches intermediate
accuracy levels with less traffic than EF-TopK and dense FedAvg.
