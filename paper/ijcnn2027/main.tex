...
pathwise state and event-budget bound and a conditional finite-horizon
gradient-norm result. A pulse-level decomposition and alignment study connect
...
accounting, Event-FedAvg is nondominated across Fashion-MNIST MLP/CNN and
CIFAR-10 CNN/ResNet-14 experiments. Across ten paired ResNet-14 seeds, it
reaches $75.14\pm0.46\%$ accuracy at $21.04\pm0.19$ Gbit, compared with
$69.47\pm0.62\%$ at $201.96$ Gbit for development-tuned dense FedAvg. These
...
algorithm. We establish pathwise state and event-budget guarantees, derive a
conditional finite-horizon gradient-norm bound, and decompose its alignment
term into measurable mechanisms. Ten-seed comparisons against four referenced
baselines use matched local training and total bidirectional traffic.
Event-FedAvg is nondominated in all four settings. On CIFAR-10 ResNet-14, it
uses $9.6\times$ less traffic than dense FedAvg while improving mean accuracy
by $5.66$ percentage points. Figure~\ref{fig:event-fedavg-method} summarizes a
complete round.}
...
\begin{theorem}[\actionrev{Conditional finite-horizon gradient-norm bound}]
...
\begin{figure*}[t]
 ...
 Dotted lines connect observed nondominated points. Error bars show mean
...
\begin{figure*}[t]
 ...
 circles 300-round intervals, and method markers round 1,800. Traffic counts
...
ResNet-14 for 1,800 rounds. Each client has 5,000 examples under the same class
...
$r_0=100$. The frozen Event-FedAvg tuples
...
difference from $\rho=1$ at the selected finite horizon.}
...
Table~\ref{tab:main-results} identifies the primary settings.}
...
separate 900-round convergence audit before its 1,800-round evaluation. For Strom and
EF-TopK, we also freeze the setting nearest Event-FedAvg in log traffic.
Table~\ref{tab:main-results} reports the primary configurations, and
Fig.~\ref{fig:communication-frontier} adds the remaining frozen points. Values
...
accuracy, parameter events, and communication.}
...
Table~\ref{tab:main-results} reports the primary frozen configurations.}
...
they are $2.4\%$, $3.1\%$, $4.1\%$, and $3.4\%$, compared with $7.4\%$,
$9.6\%$, $12.7\%$, and $10.4\%$ under conservative unicast. This sensitivity
...
\spacerev{On ResNet-14, Event-FedAvg reaches $75.14\pm0.46\%$ accuracy and
$55.2\pm3.6\%$ worst-class accuracy at $21.04\pm0.19$ Gbit. Dense FedAvg
reaches $69.47\pm0.62\%$ and $47.6\pm5.2\%$ at $201.96$ Gbit. The paired
accuracy advantage is $5.66$ points with a 95\% interval of $[5.05,6.27]$ points and
holds for all ten pairs. Thus Event-FedAvg uses $10.4\%$ of dense traffic.}
...
\spacerev{Figure~\ref{fig:resnet-communication-trajectory} shows that the
ResNet-14 advantage develops throughout training. Event-FedAvg reaches
...
although late improvement supports only a finite-horizon comparison rather
than convergence.}

\actionrev{A targeted encoder audit tests two choices at the selected compact
...
coupled schedule fails. \actionrev{Full reset and persistent accumulation are
part of the evaluated complete operator but are not isolated by this audit.
Accordingly, we do not attribute the measured gain to either choice alone.}

\spacerev{Table~\ref{tab:alignment-factorial} reports ten seeds and 31 audited
...
The remaining columns separate first-order alignment, realized descent, and
\actionrev{the sampled theorem terms and the two signed decomposition
...
all comparisons. The alignment study varies only partitioning and local steps
...
to finite-horizon progress. It is nondominated across four ten-client image
classification settings. On CIFAR-10 ResNet-14, it improves mean accuracy over
development-tuned dense FedAvg by $5.66$ points with $9.6\times$ less traffic.
