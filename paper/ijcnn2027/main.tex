...
\textbf{Operator audit.}
\actionrev{The complete Event-FedAvg operator combines cross-round encoder
memory with full reset after a threshold crossing. Baseline comparisons cannot
identify which of these internal choices matters. We therefore isolate them on
the compact CIFAR-10 CNN. A memoryless encoder discards its state before each
round, while a subtractive-reset encoder retains threshold overshoot. For each
ablated family, one threshold is selected on the development partition by
minimum log-distance to the frozen operator's total traffic. The selected
thresholds are $0.010$ for the memoryless encoder and $0.025$ for subtractive
reset. Neither is at a search-grid boundary. They are then frozen for the same
ten paired held-out seeds. All other training, quantum, and communication
...
\input{generated/p15_operator_ablation_table}

\actionrev{Table~\ref{tab:operator-ablation} asks whether the temporal state or
