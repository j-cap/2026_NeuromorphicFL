...
$E=5$ has now been completed.  Across its initial three paired seeds,
...
the strong non-IID, $E=5$ cell, first-order alignment is positive in $98.9\%$
of audited snapshots while the objective decreases in only $67.7\%$ of them.
This confirms that a useful analysis must retain both cancellation among
$P_r,L_r,B_r$ and the finite-step curvature contribution.  Extending the
frozen factorial beyond three paired seeds is the immediate empirical next
step before introducing a formal covariance assumption.
