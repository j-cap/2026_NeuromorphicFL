...
\begin{equation}
    ...
    |a_r|-(1-\rho_r)\vartheta.
\end{equation}
Because $o_r\geq0$, condition \eqref{eq:t2-event-necessary-input} follows. When $\rho_r>0$, the strict state invariant makes both inequalities strict; the non-strict form is needed for the endpoint $\rho_r=0$.
\end{proof}
...
\end{align}
The first event is correct if $\tau_+<\tau_-$ and wrong if $\tau_-<\tau_+$. The two thresholds cannot be reached simultaneously because $\vartheta>0$.
