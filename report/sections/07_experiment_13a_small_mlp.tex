...
The result should nevertheless not be overstated. The event method does not reach the centralized reference at the finite horizon. The experiment therefore supports a useful Pareto point in performance and
communication, not lossless compression of dense training.
...
\begin{center}
\resizebox{0.96\linewidth}{!}{$\displaystyle \boxed{\text{immediate monotone descent is not a necessary condition for useful nonconvex event training}.}$}
\end{center}
