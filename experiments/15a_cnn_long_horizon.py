...
for step in [0.04,0.08,0.12]:
    r=run_federated_cnn(federation=fed,method="ef_topk",n_ticks=1200,step=step,topk_fraction=0.025,eval_stride=100)
    rows.append({"configuration":f"ef_2p5_{step:g}",**r})
r=run_federated_cnn(federation=fed,method="events",n_ticks=1200,rho=0.999,gamma=1.0,threshold=0.025,jump0=0.0075,eval_stride=100)
rows.append({"configuration":"events",**r})
frame=pd.DataFrame(rows)
frame.to_csv(OUT/"long_horizon_1200.csv",index=False)
